...
**Commit at last verification:** `a44de13` (2026-08-23 13:51:28). ⭐ **THE PREVIOUSLY-UNCOMMITTED WORK IS NOW COMMITTED** — the 2026-08-22 session (BH6 AMR to bit21, metric-shadow default OFF, deflection LUT re-parameterised, playback axis unified) plus this session's four commits, on his explicit order 2026-08-23. Nothing is unpushed. Rows re-read in SOURCE at this sha this session: **C4b** (motion vectors EXIST — `render.metal:2687-2689`, `renderer.mm:695-696`), **C5/C6** (G10 measured, scanlines deleted), **C12** (`PostFXUniforms` 4-byte split, fixed + compiler-guarded), **G12** (the corpse hoist REFUTED — discard already at `render.metal:839`). **F5 / A4 / E5** closed as shipped-with-no-complaint, which is NOT measured-correct and is labelled so. ⚠️ Rows NOT re-read at this sha are unchanged and still carry their own older stamps.
...
> 🚨 **THREE BUNDLES EXIST. KNOW WHICH ONE YOU ARE LAUNCHING.** (all three re-stat'd 2026-08-23 14:12:09; the header said "two" while listing three)
>
> | Tree | Bundle stamp | Contains |
> |---|---|---|
> | ⭐ `SPACE-SYNTH-TUBE-killtube` (worktree, branch `kill-the-tube-2026-08-11`, HEAD `a44de13`) | **2026-08-23 12:15:13** | **THE LIVE ONE.** Everything through the 2026-08-23 audio/UI session (RT-thread fixes, DPI-derived UI scale, the 3840×1536 pin). Uncommitted as of 14:12:09: the bundle, `imgui.ini`, `src/main.cpp`, `src/ui/window.{h,mm}`, 3 boards + the show doc, and 2 new docs (`HANDOFF_2026-08-23_AUDIO_UI_AND_THE_ROOM.md`, `DESIGN_2026-08-23_THREE_WALL_ROOM.md`) |
