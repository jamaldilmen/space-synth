...
**Commit at last verification:** `a44de13` (2026-08-23 13:51:28). ⚠️ **THE SHA IS CURRENT BUT THE BH ROWS WERE NOT RE-READ AT IT.** This session touched audio, the envelope, post-FX and the window layer — no BH row was opened. The sha is bumped only so the board stops claiming verification at a commit that no longer exists as HEAD; treat every row below as carrying its own older stamp. ⭐ The 2026-08-22 work that used to sit uncommitted on top of `ae6ecdd` (BH6 AMR moved off the double-booked bit15 to bit21 — `SS_NO_AMR` had never worked; metric-shadow default OFF by his order; the deflection LUT re-parameterised) **is now committed and pushed.** `docs/TODO.md` carries what was actually verified this session.
