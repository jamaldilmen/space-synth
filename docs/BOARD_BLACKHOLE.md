...
- `MEASURE:` `cap=reached/landed/refused` on `accDiag[5..7]`, incremented at the tidal-test pass, after `reserved`, and at the silent `continue`. ⚠️ `accDiagBuffer` is cleared over its **first 2 words only** (`renderer.mm:2546`) — so the existing `mrg=` triple is **cumulative-since-launch, not per-frame**, and any new counter inherits that unless the clear range grows. NOT BUILT — his call.
