...
**Commit at last verification:** `8a7e890` (2026-08-26 15:53:16) — ⭐ **re-stamped 2026-08-26 21:30:00 at the handoff.** Rows re-read in SOURCE at this sha today: **L4** (blend factors, march default, blackbodyRGB), **L6** (metric-shadow default + the bit15 comment), **L9/L12/L13** (b_c re-derivation, ring spacing, the B-2 refutation), and the G6/C4b fragment return types. No other row was re-verified at this sha.
...
| **L4** | ⛔ **BOTH HALVES ARE STALE — CORRECTED 2026-08-26 15:40 (found by the BH window, verified by the brain).** **(a) "can only be orange" is DEAD:** the hardcoded `float3(1.0, 0.55, 0.25)` was replaced by `blackbodyRGB(g·T(r))` on a Shakura–Sunyaev profile with an ISCO inner edge. `blackbodyRGB` is live at `render.metal:220`. **(b) "can only be 128³ box-average" is STALE:** since 2026-07-26 13:41 the gather uses the FINE AMR grid — 128³ over ±4.0 sim = cell 0.0625, **16× finer**, ~32 samples across the disk, with coarse fallback outside the box. 🚨 **The July objection that killed the march is largely addressed IN CODE and nobody has looked since.** The march is also **DEFAULT ON since 2026-08-17 15:58:12 on his order** (`app_state.h:66`). ⭐ **WHAT IS STILL TRUE, and it is the real defect:** the march is composited **PURELY ADDITIVE** — `sourceRGBBlendFactor = One, destinationRGBBlendFactor = One` (`renderer.mm:790-791`) — and encoded AFTER the particle draw. **It is literally a second picture summed onto the first.** That is the overlay that failed in July, and under his 2026-08-26 identity clarification the fix is not to delete it but to **hand it AUTHORITY in the collapsed state** — same matter, same grid, same constants, different evaluation. | `render.metal:220`, `renderer.mm:790-791`, `app_state.h:66` (was `render.metal:3129`; `app_state.h:57`) |
...
| **L6** | **⚠️ NEW — bit15 IS DOUBLE-BOOKED and the AMR kill-switch is broken.** bit15 = "metric shadow" in the render *and* = "AMR fine force" in the physics. `physicsUniforms.bhToggles = bhToggles \| (amrOn ? 0x8000u : 0u)` — an **OR**, which can only ADD the bit, never clear it. ⛔ **STALE — CORRECTED 2026-08-26 12:0x (caught by the BH window, verified by the brain).** `uiTogMetricShadow` **defaults `false`**, and has since 2026-08-22 (`app_state.h:62`, *"DEFAULT OFF 2026-08-22"*). So in the DEFAULT state bit15 is NOT set by the shadow toggle and **`SS_NO_AMR=1` does work.** ⚠️ **The double-booking itself is REAL and still live** — the OR can still never clear the bit — so the hazard returns the moment he ticks metric shadow on for an A/B. **What is retracted is only "always set" and "any AMR A/B to date is suspect".** 🚨 **8th sighting of [[a comment is not a mechanism]], and it is a bad one:** `app_state.h:62` claims bit15 *"integrates null geodesics… so the shadow lands at the real photon-capture radius b_c = 2.598 r_s, ~2.6× the old blob."* **`0x8000` appears in `render.metal` EXACTLY ONCE, at `:963`, and all it does is cull particles at `rIn < cam.horizonR` — a 1.0 r_s cull.** No geodesic integration, no 2.598, no 2.6×. The comment describes a feature the code does not contain. | `renderer.mm:1897`, `particles.metal:2161`, `render.metal:963` (was `:821`), `app_state.h:62` |
...
| **L9** | **`bc_validate.cpp` DOES NOT EXIST IN THE TREE and never did** — `git log --all --diff-filter=A` finds no commit that ever added it. `render.metal:2990` and `app_state.h:53` both cite it as the integrator's validation (b_c to 1.4e-6). **That validation is currently an unverifiable claim.** Re-deriving it is cheap and should be done before the march is ever trusted again. | verified 2026-08-14 ✅ **CLOSED 2026-08-26 15:52 — RE-DERIVED FROM SCRATCH, AND WE BEAT THE CLAIM WE COULD NEVER FIND.** Two methods, only one sharing algebra with the renderer. **(A) analytic:** photon sphere r = 1.5 r_s ⇒ b_c = **2.59807621135332**, error 0.00e+00. **(C) INDEPENDENT Cartesian null-geodesic ODE** — `d²x/dλ² = −(3/2)·r_s·h²·x/r⁵`, RK4 at constant angular step, capture at the horizon, bisected on b, converged in step size ⇒ **b_c = 2.598076203, absolute error 8.8e-9, relative 3.4e-9.** h conserved to 1e-12 throughout. ⭐ **Cross-check of α(b), ODE vs the renderer's own quadrature: 3e-8 to 1e-7 relative across 2.60 ≤ b ≤ 10, including 9.1e-8 at exactly b = 2.62.** **The never-existent `bc_validate.cpp` claimed 1.4e-6; we are ~400× better than the claim it made.** `[MEASURED n=2 independent methods + step-size convergence]` |
| **L12** | 🆕🔬 **THE PHOTON-RING STACK IS SUB-PIXEL — R2 AS ORIGINALLY WORDED IS GEOMETRICALLY UNREACHABLE, MEASURED 2026-08-26 15:52.** At our live scale (r_h = 0.3125 sim, ρ=150, 3024×1898 ⇒ 1 r_s = 164.76 px): direct image at **b−b_c = 8.04e-02 r_s = 13.25 px** (resolvable); **n=1 winding at 1.41e-04 r_s = 0.023 px — 43× SMALLER THAN ONE PIXEL**; n=2 at 2.62e-07 r_s; each ring **e^(2π) = 535.5× closer** than the last (measured ratios 570 → 538 → 534.7, converging on theory; brain re-verified the arithmetic independently). 🚨 **No renderer at 1898 px resolves this and NASA's does not either** — in their frame the photon ring IS the bright rim, and *"orbited two, three or more times"* is an ANALYTIC description of what is inside it, not resolved bands. ⭐ **RE-SCOPED TARGET: a thin bright rim at b_c with brightness falling off inward across the ~13 px to the direct image** — exactly the band our profile measured DEAD FLAT (0.201–0.228, no trend). ⛔ The bible's R2 row was written by the brain without checking resolvability; corrected in `docs/reference/BH_REFERENCE.md`. | `[MEASURED]` at our own scale + independent arithmetic re-check |
| **L13** | 🔴 **THE SPRITE-LENS PATH CANNOT CARRY RADIAL STRUCTURE NEAR b_c WITH THIS SOLVER — measured 2026-08-26 15:52, and it kills the cheap option.** B-2 tried the approved fallback (guard at b_c+1e-4 instead of the 2.62 floor). Three matched settled pairs at r_h=0.3125, background identical to 4 d.p. so only lensed features moved. **The spike DID die (0.291→0.223, to background) but NO gradient replaced it:** the 1.82–2.67 r_s band went **DOWN 5.1%**, still flat; the 3.06 r_s feature **lost 90%**; **total flux fell 13–17% in every pair. Images are CULLED, not redistributed.** ⭐ **MECHANISM: the clamp was doing real NUMERICAL duty, not just acting as a floor.** α(2.62) = 4.396 rad but α(2.5981) ≈ 12 rad — 3× larger; an overshooting Newton iterate lands where α is enormous, the solve diverges, thEff collapses below the capture radius and `:1161` culls it. That is why light at 3.06 r_s died too. ⟹ **(b) does NOT get the cheap annulus-only option on the strength of a salvageable sprite path. If the rim falloff is wanted, the integrator must own that band.** ⚠️ **The honest retry is NOT moving the clamp — it is making the solve robust: iterate in `log(b − b_c)` where α is smooth, instead of in b where it diverges.** | `[MEASURED n=3 matched pairs]` |
