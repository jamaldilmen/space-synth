...
**Commit at last verification:** `641aa8f` ⭐ **RE-STAMPED 2026-09-05 15:28:30 (BRAIN, /handoff) — HEAD. The source commits since `41609e1` are `8182846` (theta nearest-representative — an absolute target past π spun the camera a full turn every 7 frames), `7ea4fe8` (`SS_CAM_THETA`/`SS_CAM_PHI` launch envs), `acda80f` (14-bit exposure pair CC22+CC54) and `23222fc` (generated-take tools into `scratchpad/`) — all `src/main.cpp` + `src/core/camera.h`, camera/control path, **NO .metal, NO renderer.mm, no BH mechanism**. §AA29 (FABLE) and §AA30 (BRAIN) are this session and are verified at this sha; **NOTHING else below was re-verified.** 🚨 **§AA30.1 IS THE ROW TO READ FIRST: in PERSPECTIVE do not zoom past rho ~2925** — the §AA28 near-plane fix bought a FACTOR, not immunity (depth step = ulp·z²/near ⇒ 0.51 units at 2925, **2.00 at 5800**); ORTHO is exempt. 📦 18 DXV3 delivered, six takes, 33 minutes — and **the set spans TWO binaries** (§AA30.5).** Previously `41609e1` ⭐ **RE-STAMPED 2026-09-05 11:58:00 (FABLE, /handoff) — HEAD. Source commits since `a66c63c`: `0cac109` (kMaxRho 2000→5800), `1460d40` (camera target getters), `a94e76f` (SS_ORTHO + SS_LUM_CEIL env), `f09609f` ([CAMF]/[CAM-LIVE] instrument), `38170d5` (**THE FIX: perspective near plane 0.001→1.0, far 5000→20000**) — all `src/main.cpp` + `src/core/camera.h`, control path, NO .metal, NO renderer.mm, no BH mechanism. `25bbfc7` was a two-window collision (FABLE staged over BRAIN's commits) and is REVERTED by `41609e1`; `91604cb` is imgui.ini. §AA28 is this session and is verified at this sha; NOTHING else below was re-verified.** Previously `a66c63c` ⭐ **RE-STAMPED 2026-09-04 19:01:04 (OPUS, /handoff) — HEAD. Source commits since `0efe13d`: `f0cac86` (CC-driven rigid spin, `src/main.cpp` + `src/core/app_state.h` — control path only, no .metal, no renderer.mm, no BH mechanism) and `a66c63c` (a scratchpad tool, no app code). §AA27 is this session and it is verified at this sha; NOTHING else below was re-verified. Detail is in `docs/SPACE_SYNTH_LIVE.md`.** Previously `0efe13d` 📖 **READ `docs/SPACE_SYNTH_LIVE.md` FIRST for anything render-shaped — it is the show bible (his order 2026-09-04 15:12:04) and it holds the scale, the DXV3 delivery pipeline, the render economics and every learning with what it cost. §AA26 below is its board-side summary; S6 SHIPPED (`14757d5` + `ff1fdc0`), DXV3 is ~3× SMALLER than ProRes and PROVEN in Resolume, and TAKE 4 IS BUILT BUT NEVER RUN — synthetic-log verified only.** ⭐ **RE-STAMPED 2026-09-04 15:22:06 (FABLE, /handoff) — HEAD. The ONLY source commit since `85b4aa3` is `ff1fdc0` (SONNET 15:18:09, midi CC ride mapping: `src/main.cpp` + `src/core/camera.h`, control path, no .metal/renderer.mm). Its content IS the S6 arm of §AG.1 row 2 (measured: PINNED 0/3, UNPINNED 3/3) — so §AG is verified against it; nothing else re-verified at this sha.** Previously `85b4aa3` (FABLE 15:19:48) — docs-only sha; ONE pointer added at §AA25 → `BOARD_BLACKHOLE.md` §AG (formation = path × binary layout, not the pin). Nothing else in this file re-verified at this stamp.** Previously `e750a73` ⭐ **RE-STAMPED 2026-09-04 02:38:15 (BRAIN) — §AA16–AA25 hold the night of 2026-09-04. ⚠️ BRAIN and FABLE folded this file INDEPENDENTLY and BOTH folds are in: AA16–AA22 are BRAIN's, AA23–AA25 are FABLE's (renumbered 2026-09-04 02:38:15 — they were a second AA16/AA17/AA18 and a cold start grepping a number got two different rows). Same night, two voices, no loss — see §AA22. 🚨 **READ `BOARD_BLACKHOLE.md` §AF FIRST: THE PINNED RENDER PATH NEVER FORMS THE HOLE AT LAUNCH — PINNED 0/9, UNPINNED 8/8, same binary, same spawn (FABLE, MEASURED).** His *"the bh didnt survive the scaling"* was RIGHT and BRAIN's "not the pin" ruling is RETRACTED (§AA22). S8 renders PINNED ⇒ this is a SHOW BLOCKER. 🎛️ **HIS PARAMETER LIST LANDED — S6 IS UNBLOCKED (§AA18): zoom · camera up/down · exposure · fluid · glitch · pause · chromatic**, and `pause` is a **SWITCH, not a 0–127 fader** — a type the registry does not have. 🎬 **THE DELIVERABLE IS THREE 10-MINUTE PARTS (§AA20)**, ~180 GB each against his **466 GB free**; the SIM PAUSE is his named **TIME GLITCH**, and ⚠️ trails are FORCED OFF while paused (`main.cpp:2550`). ✅ Bloom pyramid FIXED `bea0b0d` — *"OKAY IT LOOKS BETTER"*, costs nothing (§AA16). ✅ Smear taps per-device-pixel `e750a73` — **UNVERDICTED** (§AA17). 📐 `sizeResScale` has ONE consumer ⇒ four UV radial effects are ellipses at 11.69:1, dormant (§AA21). ⚠️ SCOPE: §AA16–AA22 verified at this sha; nothing else below re-verified.** Previously `3e085d7` ⭐ **RE-STAMPED 2026-09-03 22:25:19 (BRAIN) — S8 SHIPPED, RAN AND PASSED: three ProRes wall slices, 300 frames each, verified by ffprobe; FULL RES IS DELIVERABLE (≥43 fps at 19,644×1680, 2.56 GB peak) — §AA10/AA12. 💾 A 30-MINUTE SET = 540 GB (§AA13, his ruling: he deletes to fit). 👁️ HIS EYES ON THE OUTPUT: *"Test looks amazing. Bravo."* — the LOOK IS ACCEPTED (§AA14); the side walls being near-black is COMPOSITION, his call. 🎛️ S6 IS NOW THE BLOCKER for the live-preview workflow, S7 is ONE universal slew law, and an ABLETON CC LANE CANNOT BE RENAMED ⇒ a generated M4L device is required (§AA15). ⚠️ SCOPE: §AA was verified at this sha; nothing below it was re-verified.** Previously `52f6d68` ⭐ **RE-STAMPED 2026-09-03 18:40:11 (BRAIN) — THE SHOW RENDERER IS FOLDED IN AS §AA (read it FIRST). S1–S5 shipped; **0 frames rendered**; **19,644 can never be the on-screen drawable** (the presenting CAMetalLayer refuses >16,384 — SIGSEGV, his *"it crashed lol"*), so S8 must capture from an OFFSCREEN target. ⛔ FABLE's *"a replayed event is never early"* is RETRACTED — it led the live take by 11–31 ms; `ceil` fixed it in `d6cbb7c`. 📍 §AA9 lists the hunk boundaries: `main.cpp` +130/−15 and `renderer.mm` +66/−3 since the previous stamp, PIECEWISE — re-read every citation above them. ⚠️ SCOPE OF THIS STAMP: §AA was verified at this sha; the 168 KB below it was NOT re-verified and every other row still carries its own older stamp.** `52f6d68` is the last SOURCE commit; docs-only commits after it carry no source change. Previously `74bee76` ⭐ **LINE ADDED 2026-09-03 16:02:29 (BRAIN, on his order "fix the board"). This file had NO verification line at all until now — preflight had been warning on it, and a board with no stamp cannot be caught lying.**
⚠️ **READ THIS BEFORE TRUSTING A ROW: the sha above records the tree state at which this LINE was added, NOT a re-verification of the 167 KB below it.** Every row still carries its own, older stamp, and many describe code that has since moved or been deleted (the BH-renderer rows in §V in particular — both renderers were deleted 2026-08-27). **Re-grep any `file:line` here before acting on it; never re-derive by arithmetic** ([[space_synth_board_is_the_reference]], and `BOARD_BLACKHOLE.md` §AE.7 — `renderer.mm` moved +22 and `main.cpp` +11 mid-session on 2026-09-03 while four windows were citing them).
🖤 **The BLACK HOLE board is `docs/BOARD_BLACKHOLE.md` and it is the one that is current** — stamped at `74bee76` with the night of 2026-09-03 folded in as §AE. This file is the ENGINE-WIDE board: clock, grid, camera, colour, audio, UI. When the two disagree about black-hole state, `BOARD_BLACKHOLE.md` wins.
...
## AA. 🎬 SESSION 2026-09-03 EVENING — **THE SHOW RENDERER: S1–S5 SHIPPED, AND 19,644 CAN NEVER BE THE ON-SCREEN DRAWABLE.**

**His order, ~16:00 via BRAIN, verbatim `[HIS WORDS]`:** *"I want 60 fps and 30 fps. Full res. And half res options … 3 slices, front wall 5340 x 1680, 2x side walls 7152 x 1680 … The midi cc must work so well that i can compose rides and fades accurately. Set both melodies and chords and the midi cc as automations in ableton. Have the app 'read' the midi and render accordingly. **This is the most important build of the project.**"*
**Built by FABLE 17:40–18:32, one concern per commit. Folded by BRAIN 2026-09-03 18:40:11.** Full state, env vars and test tooling: memory `space_synth_show_renderer_2026-09-03` and `docs/HANDOFF_2026-09-03_FABLE_RENDERER.md`.

| # | Step | What it does | Where | Proof |
|---|---|---|---|---|
| AA1 | **S1 parser** `d4cf127` | CC + channel parsed; System Common **sized** (`0xF2`→3, `0xF1`/`0xF3`→2, rest →1; SysEx scans to `0xF7`; `>=0xF8`→1, the `9fbe0ba` guarantee unchanged). One POD callback: `MidiEvent{kind,channel,a,b,stamped,t}`, `t` in `CACurrentMediaTime` seconds from the **packet's own stamp** | `src/core/midi_input.h:11-26`, `midi_input.mm:44-64` | `[READ]` BRAIN 18:31 · `[MEASURED n=17 packet cases + live IAC]` FABLE |
| AA2 | **S2 recorder** `7ff7158` | `SS_RECORD=<path>` logs every note+CC with packet time; **marker CC 119** (`SS_TAKE_MARKER_CC`) = t0; per-frame envelope rows are the ground truth S9 is checked against. A take with **no marker fails loudly**; drops are counted in the header | `src/core/take_recorder.h` + `src/core/take_recorder.cpp` | `[MEASURED n=2 live takes]` 42 events / 1,537 frames, FABLE |
| AA3 | **S3 offline clock** `5fd6cbb` | `SS_RENDER_FPS=30\|60` ⇒ `dt = 1/60` **exactly**, 2 steps per output frame at 30, 1 at 60 ⇒ sim seconds per output frame = `1/fps` exactly. Warp **pinned to 1 and logged**, never assumed. Gates the physics step size + count, substeps, the posed-disk clock, the lens EMA, the window frame `dt` (sequencer / camera / VJ crossfade) and `main.cpp`'s second `dt` copy. **One env read, one gate; unset ⇒ every consumer takes its original line.** `SS_RENDER_FPS=25` is **refused loudly**, never silently approximated | `src/core/offline_clock.h`; gates in `renderer.mm`, `main.cpp`, `window.mm` | `[READ]` BRAIN 18:33 · `[MEASURED]` `frame=300 simTime=10.000020 expected=10.000000` (2e-5 = float32 accumulation in the shader time uniform; the step **count** is integer-exact) · live run pre/post identical, 0 `[OFFLINE]` lines with the flag unset, FABLE |
| AA4 | **S4 replay** `7d2e0d8` + **ceil fix** `d6cbb7c` | `SS_REPLAY=<take>` applies events at output frame `SS_REPLAY_START_FRAME + ceil(t·fps)`, at the top of the frame, **through the same `onMidi` callable live packets use** — one code path. Refuses: clock off, `t0 NONE`, `dropped > 0` | `src/core/take_replay.h` + `src/core/take_replay.cpp` | `[MEASURED n=2 identical runs]` `[REPLAY]` output byte-identical, 40/40 events at the frame law, FABLE |
| AA5 | **S9 envelope** `3e4ac40` | Offline the audio engine is **not started**; the frame callback drives `processBlock(48000, …, 1600)` once per output frame (48000/30 exactly), after that frame's notes are queued and before the envelope is read. Audio is discarded — Ableton has it | `main.cpp` (S9 hunk) | `[MEASURED]` vs the S2 live take: every phase transition within one frame, phase 362/363 rows (99.7%), median \|amp err\| 0.0000, p95 0.0048; two replays byte-identical on all 466 rows, FABLE |
| AA6 | **S5 resolution pin** `52f6d68` | The 16,384 constant became a **measured** `Window::canAllocateDrawable()` (allocates on the device, then asks a `CAMetalLayer` for the drawable; refused loudly, never a silent 1280×800). A pinned render's sprite reference is the **delivery height 1680** (`SS_REF_HEIGHT` overrides); live keeps 2260 | `renderer.h`, `window.h`, `window.mm` | `[MEASURED]` 9822×840 → reference 1680 → scale 0.5000, whole chain runs at **21.6 fps / 0.71× real time** at 2M particles; live untouched at 2260 / 0.7080, FABLE |

✅ **AA7 — THE FULL-RES BLOCKER — CLOSED 2026-09-03 20:22:11 BY §AA10. It was a LAYER limit, not a memory limit.** `[MEASURED 18:22–18:25, FABLE]` Every offscreen texture type allocates **and rasterizes** at 19,644×1680 (RGBA16F mipmapped, RGBA16F, Depth32F, RG16F, RGBA8; clear+readback exact) and ProRes 422HQ/4444/HEVC sessions open at that size — but **the presenting `CAMetalLayer` refuses any drawable above 16,384**: `nextDrawable` nil, `drawableSize` reads 0×0, and the frame's final blit `copyFromTexture:drawable.texture` dereferenced nil ⇒ **SIGSEGV** (crash report 18:22:51; his words *"it crashed lol"*). Metal's own descriptor validation **aborts** above 32,768 (report 18:24:10). ⇒ **S8 must render the final post pass into an OFFSCREEN target of the pinned size and present a ≤16,384 preview; the capture reads the offscreen target.** Guard built 18:30:36, **NOT LAUNCHED** — his interrupt on the launch stands. **Half res works end to end today.**

⛔ **AA8 — RETRACTION, FABLE's, published to the team at 18:13 before it was caught.** S4 shipped `floor(t·fps)` claiming *"never early, at most one frame late"*. **It is the opposite** — measured against the live take, the replay **LED** it by 11–31 ms on every envelope transition, and video leading audio is the direction the eye catches. Fixed to `ceil` in `d6cbb7c`; `[MEASURED]` transitions now lag ≤ 1 frame. **The only alignment claim allowed today: each event lands 0–33 ms AFTER its recorded time, never before, deterministic to the frame — on a SYNTHETIC take. Ableton's own MIDI output jitter is UNMEASURED until his first real take** (the take header reports `stamps packet|callback|mixed`).

📍 **AA9 — LINE-NUMBER DRIFT SINCE THE `74bee76` STAMP. Re-read, never renumber by arithmetic** ([[feedback_file_line_is_only_true_against_a_tree_state]]). `git diff --numstat 74bee76..52f6d68 -- src/`: `main.cpp` **+130/−15**, `renderer.mm` **+66/−3**, `midi_input.mm` **+62/−14**, `midi_input.h` **+20/−3**, `window.mm` **+52**, `window.h` **+5**, `renderer.h` **+7**, plus four new files. The offsets are **piecewise per hunk, not uniform** — `renderer.mm` hunks open at old `2`, `333`, `1713`, `1784`, `1923`, `2156`, `2178`, `2326`, `2333`, `5041`; `main.cpp` at old `5`, `122`, `125`, `130`, `155`, `204`, `213`, `222`, `641`, `2651`, `2736`, `2738`, `3117`. **Every `renderer.mm` and `main.cpp` citation elsewhere on this board that was last read before 17:40 is suspect above those boundaries.**

| AA10 | **S8 capture — SHIPPED** `c6cad75` | The render size is pinned **apart from the layer** (`pinRenderSize()`; `resize()` then ignores its arguments). With a pin: the final post pass goes to an **offscreen** `finalTexture` (EDR, feeds `prevFrameTexture` exactly as the drawable did), a second pass at headroom 1.0 into `captureTexture` (what the projector sees), a third pass at the **drawable's** size into the drawable as a **preview**, then one blit per slice into that frame's pixel buffers; after commit, `waitUntilCompleted` + append (offline only). Buffers are **64RGBAHalf IOSurface wrapped as RGBA16F via `CVMetalTextureCache`** — the same format as the render targets, no conversion. Three `AVAssetWriter`s, ProRes 422 HQ / 709, cropping **x 0–7151 · 7152–12491 · 12492–19643**; `pts = frame/fps`; back-pressure = wait on `isReadyForMoreMediaData`. **Refuses** without the offline clock, and refuses slice widths that don't sum to the render width. Drawable = the largest exact half `canAllocateDrawable()` accepts (19,644 ⇒ **9,822×840 preview**, same aspect; the camera aspect reads the drawable). `SS_CAPTURE` arms it, `SS_CAPTURE_SLICES`, `SS_CAPTURE_FRAMES` ends the run. **Live path: pin unset ⇒ nil targets, byte-for-byte the old code.** | `src/render/show_capture.h` + `src/render/show_capture.mm`; pin + passes in `src/render/renderer.mm`, `src/render/renderer.h`; gates in `src/main.cpp`, `src/ui/window.mm` | `[MEASURED run 2026-09-03 20:22:04→20:22:11, FABLE; re-verified independently by BRAIN 20:25:35]` 300 frames written, **app quit itself, exit 0, no crash report**; `ffprobe` **prores / yuv422p10le · 7152,1680,300 · 5340,1680,300 · 7152,1680,300** |
| AA11 | **Two lying log strings** `3e085d7` | `[REPLAY] ARMED` printed `floor(t*fps)` while the schedule has been `ceil` since `d6cbb7c`; `[SIZE]` called the pinned **render** size "drawable" when the drawable is the 9,822×840 preview. **Strings only** — no maths touched (`take_replay.cpp` diff is 1 line) | `src/core/take_replay.cpp:75`, `src/render/renderer.mm` (`[SIZE]` printf) | `[READ]` BRAIN 20:58:36 at `3e085d7`: `git diff` is 2 lines; `strings` on the built binary = `ceil(t` ×1, `floor(t` ×0 |

⚡ **AA12 — FULL RES IS DELIVERABLE ON THIS MACHINE. `[MEASURED 2026-09-03 20:22, FABLE]`** 300 frames **rendered and written** at **19,644×1680** in **≤7 s wall** (launch→last file closed, startup included) ⇒ **≥43 fps, faster than real time**, at 2M particles with three post passes plus the writer. The app's own counter across the run: 16 (warm-up) → 49 → 71 → 76 → 80. `[PROFILE/120f]` Compute avg 11.11 ms · Render+PostFX avg 9.26 · **Total avg 20.36 ms, max 78.50**. Peak **RSS 2.56 GB** (1-s `ps` samples; RSS, not footprint). ⚠️ **The caveat that bounds all of it: this was the S2 SYNTHETIC take with `hole 0%` — the lens pass cost at 19,644 wide is UNMEASURED**, and a 30-minute live soak (memory growth, MIDI drops, frame-time hold) has never been run.

💾 **AA13 — THE SIZE LAW, and it decides his disk. `[MEASURED from the three real files, BRAIN 21:16:38]`** L 982,726,939 B · C 1,050,482,859 B · R 967,337,195 B for **10.000 s / 300 frames** = **3,000,546,993 B ⇒ 300 MB/s (2.4 Gbps) for all three walls**. His set is **30 minutes** (`[HIS WORDS]` 2026-09-03 21:0x: *"The set is 30 mins"*) = 180× that ⇒ **540 GB (503 GiB)**: L 177 · C 189 · R 174. Free at the measurement: 366 GB ⇒ **it does not fit; ~180 GB must be cleared**, and that is a FLOOR — ProRes is VBR and this take had two near-black walls, so real content encodes **bigger**. His ruling `[HIS WORDS]` 2026-09-03 ~21:1x: *"Nah it's cool it'll fit. I'll delete some stuff."* ⇒ **codec stays ProRes 422 HQ; no LT, no external drive, not discussed further.**

👁️ **AA14 — HIS EYES ON THE OUTPUT: ACCEPTED. `[HIS WORDS]` 2026-09-03 ~21:2x — *"Test looks amazing. Bravo."*** — on the frame-200 wall composite (L|C|R at 1/8, sent by BRAIN). ⚠️ **What the data says about that frame, and it is a COMPOSITION question, not a bug:** `ffmpeg signalstats` 10-bit luma — C: YAVG 93.7 / 68.1 / 73.5 and YMAX 573 / 785 / 928 at frames 0/200/299; **L and R: YAVG 64.6 (video black IS 64), YMAX 400–650** ⇒ **the matter sits in the centre 5,340 of the 19,644 and the side walls are nearly empty in this take.** The camera aspect is the drawable's 11.69:1 by construction — identical to the render. **Filling the side walls is his call, not a defect.**

🎛️ **AA15 — S6/S7 REQUIREMENTS HE ADDED TONIGHT — read before anyone specs the registry.**
- `[HIS WORDS]` 2026-09-03 ~21:5x: *"It doesnt matter. Every fader gets the same movement. I'll list all parameters that need mod support when I'm home in front of screen."* ⇒ **S7 is ONE universal SLEW law across every mapped fader — a per-parameter SLEW table is dead before it was designed.** His parameter list is pending and is HIS to write. ⛔ **DO NOT READ THAT AS KILLING THE VALUE CURVE — it is a different quantity** (slew = temporal smoothing between event stream and parameter; curve = how 0..127 maps onto min..max). `[READ src/main.cpp:41-42, :60-61 @ 58f70b8, OPUS's catch, re-verified by BRAIN 22:31:33]` both helpers already take `ImGuiSliderFlags flags = 0`, so `Mapping::curve` is **INHERITED from each widget's own flags, never tabled and never hand-synced** — the anti-drift argument that put it there. Deleting that field drives **NINE `ImGuiSliderFlags_Logarithmic` faders** (counted at `58f70b8`; the design said 8, it grew by one) linearly — `uiIscoSeconds` (`main.cpp:1660`) spans **0.02–30 s, 1500×**, where 128 linear steps is worthless at the bottom and a cliff at the top.
- `[HIS WORDS]` 2026-09-03 ~22:0x: *"what I wanna do is have my set in one session. The entire 30 mins. Press play. Send it to space synth. And watch it render. I need a real time preview. So I can arrange my audio and visuals accordingly."* ⇒ the target workflow is the **LIVE path with mapping**, not replay-only. The live half already exists (CC parses since `d4cf127`, `SS_RECORD` logs the take); **S6 is its blocker — CC arrives and is printed but moves NOTHING today.** ⚠️ Live steps at 60.606/s on wall time while offline steps at exactly `fps×2` with warp pinned: `[MEASURED over 10 s]` they agree to ≤1 frame on every envelope transition (362/363 rows) — **over 30 minutes this is UNMEASURED, and the system is chaotic ⇒ preview ≈ final, not = final.**
- 🇦 **ABLETON SIDE, verified against Ableton's own docs 2026-09-03 ~22:1x (BRAIN), NOT from memory:** a MIDI clip envelope's Device chooser → **"MIDI Ctrl"** + the Control chooser picks the CC; the output port's **Track** switch (Preferences → Link, Tempo & MIDI) is what transmits **notes AND CC** (Sync = MIDI clock, Remote = mappings/feedback). **Live offers controllers up to 119 ⇒ CC 119 IS drawable in a clip envelope and it is our take marker** — the registry must refuse it (`SS_TAKE_MARKER_CC` moves it) or a drawn 119 lane is eaten as a marker. **CC 0 and CC 32 are NOT offered** (bank select). 🚨 **A CC lane CANNOT be renamed in Live — a decade-old, still-open request.** ⇒ to satisfy his *"I see the name of the parameter I'm changing in Ableton's automation view"* and *"values need to make sense"*, the rides must be drawn on a **Max for Live device** whose parameters carry Long/Short names, type, min/max and unit style (Ableton's own M4L guidelines require non-generic names + "Automated and Stored" visibility) — and **that device must be GENERATED from the app's own registry** (every fader already carries label/min/max/curve at its `UiSliderFloat`/`UiSliderInt` call site), never hand-typed in two places. **Live has no native 14-bit CC lane** (only CC n + n+32 as two separate CCs) ⇒ **the wire stays 7-bit ⇒ slew is not optional.** 14-bit is possible only because the M4L device would be ours to write; **recorded as an option, NOT chosen.**

| AA16 | **Bloom pyramid depth — FIXED** `bea0b0d` | At the pinned 19,644×1680 the pyramid stopped at **7 levels, coarsest 153×13**: the widest glow scale was 1/13 of the frame HEIGHT but only **1/153 of its WIDTH**. The width grew 6.5× over the laptop view, the height did not, and the break keyed on the **SHORTER** side ⇒ no extra levels. **TWO independent blocks, both had to move:** `renderer.mm:6070` `(lw<8 \|\| lh<8)` → `(lw<8 && lh<8)`, and `renderer.mm:96` `kBloomMaxLevels` **8→16** (19,644 needs 11). ⛔ The kernels were NEVER the bug — `postfx.metal:684`/`:736` take offsets in SOURCE texels, scale-invariant per level. No new uniform; `PostFXUniforms` untouched. | `src/render/renderer.mm:96`, `:6070`, `:6083` | `[MEASURED]` from the running process, before `[BLOOM-PYRAMID] 7 levels from 9822x840`, after `11 levels … down to 9x1`; widest scale 1/153 → **1/9** of the width. **Costs nothing:** like-for-like quiet/hole<50% **18.4 fps (n=233) → 18.5 (n=85)**. `[HIS WORDS 2026-09-04 ~00:0x]` *"OKAY IT LOOKS BETTER"* |
| AA17 | **Smear tap count — SHIPPED, UNVERDICTED** `e750a73` | `SMEAR_TAPS` was a fixed **48** over a streak length that scales with resolution ⇒ at 19,644 the taps sat ~6.5× further apart in pixels and a continuous band became a dotted trail. Now one tap per DEVICE pixel: `lenPx = length(mv*u.resolution)`, `taps = clamp(ceil(lenPx),1,256)`; decay renormalised `pow(decay, t*48)` so the hold dial keeps its old meaning. | `src/render/postfx.metal` (smear block, was `:272`) | `[READ]` diff +24/−5, one concern. 🚨 **HE HAS NOT SEEN IT** — bundle rebuilt 02:29:58 CONTAINS it; the app he watched (PID 81680, 02:20:56) does NOT. **Cost at 256 taps under play with the hole up is UNMEASURED.** |
| AA18 | 🎛️ **HIS PARAMETER LIST — S6 IS UNBLOCKED** | `[HIS WORDS 2026-09-04 ~02:3x]` *"midis i want. zoom .. camera up and down ... exposure ... fluid....glitch .. pause.. must be a on off switch in midi not a 1-127 value... chromatic as a fader too"*. ⇒ **seven parameters: zoom · camera up/down · exposure · fluid · glitch · pause · chromatic.** 🚨 **THE REGISTRY NEEDS TWO TYPES, NOT ONE — `pause` is a SWITCH, not a 0–127 fader.** S6 was designed as continuous-only; a toggle needs its own kind (threshold + hysteresis), and `simPaused` is a bool at `main.cpp:678`/`:2898`, not a slider, so it is NOT reachable through `UiSliderFloat` and has **no** `Mapping` entry today. `chromatic` is explicitly a fader. | his words; `main.cpp:678`, `:2898` | `[HIS WORDS]` — **the list that S6/S7 were held behind is now GIVEN.** |
| AA19 | 🎹 **AND THE ABLETON SIDE OF IT, restated as a requirement** | `[HIS WORDS 2026-09-04 ~02:3x]` *"the infos that midi shows in ableton must make sense basically like a soft synth that mirrors space synth but in a language thats automatable within ableton automations"*. ⇒ confirms §AA15: a CC lane cannot be renamed in Live, so the rides are drawn on a **Max for Live device GENERATED from the app's registry**, carrying Long/Short names, type, min/max and unit style — **a soft-synth mirror of Space Synth, not a CC number sheet.** | §AA15 | `[HIS WORDS]` |

🎬 **AA20 — THE SHOW SHAPE AND THE TIME GLITCH, both HIS.** `[HIS WORDS 2026-09-04 ~00:1x]` *"three parts each 10 mins. waaaaaaaaaaay more digestible. drift wont get toooooo crazy. more easy to change stuff on the go. edit it if its shit. re render if somethings not adding up then waiting for a full 30 min file every time."* ⇒ **the deliverable is THREE 10-MINUTE PARTS**; the live-vs-offline agreement that was UNMEASURED over 30 min is now a **10-minute** question. `[MEASURED §AA13]` 300 MB/s ⇒ **one 10-min part ≈ 180 GB**; `[HIS WORDS ~02:3x]` *"i have 466 gigs free"* and he authorises spending it ⇒ **one part fits with room; three at once (540 GB) do not — parts must be offloaded between renders.** ⭐ **AND THE DEVICE HE NAMED:** *"this is gonna be our TIME GLITCH for rendering phase"* — the SIM PAUSE is show vocabulary, not an optimisation. `[READ main.cpp:2898]` `if (!simPaused) { renderer.computeStep(...) }` ⇒ the whole compute step is skipped while the render loop runs; SPACE toggles it (`:678`, prints `[SIM] PAUSED`). 🚩 `[READ main.cpp:2550]` `config.trailDecay = simPaused ? 0.0f : app.uiTrailDecay` — **trails are FORCED OFF while paused**, so a ride on a paused sim does not look like a ride on a live one. ⚠️ **THE FPS GAIN IS UNMEASURED and must not be estimated:** §AA12's Compute 11.11 / Render+PostFX 9.26 ms split is the **hole 0%** synthetic take, while at venue res with a hole up the app ran 6–13 fps and the lens and smear are RENDER cost. Measuring it needs HIM at the keyboard (run → supernova → SPACE → ~15 s → SPACE); `[SIM] PAUSED` brackets the FPS lines.

📐 **AA21 — RESOLUTION-TRAP AUDIT: `sizeResScale` HAS EXACTLY ONE CONSUMER.** `[READ render.metal:2525]` `out.pointSize *= max(cam.sizeResScale,1e-3f)` is the ONLY place the render normalises to resolution. Everything else in the post chain is pixel- or UV-space and does not scale. `u.resolution` IS passed in `PostFXUniforms` but **is never used for geometry**. ⇒ **dormant traps, all default 0, none fired in his runs, RECORD-ONLY:** vignette `postfx.metal:545`, chromatic aberration `:192`, kaleidoscope `:155`, twirl `:165` — each computes a circle in raw UV, which at **11.69:1 is an ellipse**. ⚠️ Vignette would darken the far ends of the side walls, which touches §AA14 — tell him before anyone turns it on. 🚩 **AND ONE THAT IS NOT DORMANT:** `[READ renderer.mm:5410]` `post.pixelStretch = min(1, max(config.pixelStretch, bhStrength))` ⇒ **while the hole is up the smear runs at full strength regardless of his fader**; only `smearShutter → 0` kills it. The `max()` is his deliberate routing — **his call whether it stays, NOT changed.**

**Still unbuilt — ⭐ S6 IS NO LONGER BLOCKED, his list landed at §AA18:** **S6** the CC→parameter registry from `docs/DESIGN_2026-09-03_MIDI_MAP_AND_LINK.md` (apply **outside** `if (showHUD)`; refuse mapping the marker CC) — **now the blocker for the workflow he described in AA15** → **S7** slew on the frame clock, ONE law for every fader. **On his go, and behind his parameter list.**


**AA23 — ✅ BLOOM SCALED WRONG AT FULL SIZE — FIXED `bea0b0d`, HIS VERDICT *"OKAY IT LOOKS BETTER"* (2026-09-04 ~00:0x).** `[HIS WORDS 2026-09-03 23:5x]` *"bloom scaled wrong with the full size scale."* `[READ renderer.mm resize(), kBloomMaxLevels]` The pyramid halved while the SHORTER side stayed ≥ 8 px AND was capped at 8 slots; at 19,644×1680 that gave 7 levels / coarsest 153×13 — in absolute px the glow was unchanged (128 px) but frame-relative reach fell 1/23 → 1/153 of the width. Now halves while the LONGER side ≥ 8, 16 slots, **11 levels down to 9×1** `[MEASURED from the process: [BLOOM-PYRAMID] 11 levels from 9822x840 down to 9x1]`; both blocks had to move. Cost 18.4 → 18.5 fps (BRAIN's like-for-like). No new uniform.

**AA24 — 🔧 SMEAR TAPS — WRITTEN, COMPILED, COMMITTED `e750a73` ON /handoff, UNVERDICTED, NOT IN A VERDICT RUN.** `[READ postfx.metal smear block]` `SMEAR_TAPS = 48` was a fixed count over a UV-length streak ⇒ 6.5× wider tap spacing in px at 19,644 ⇒ dotted trail. Now `taps = clamp(ceil(length(mv·resolution)), 1, 256)`, decay exponent normalised to the old 48-tap span. Cost coverage-bound (attachment 1 cleared each frame). 🚩 **Flag, his call:** `renderer.mm` `post.pixelStretch = min(1, max(spin, bhStrength))` — with the hole up the spin fader cannot turn the smear off. 📋 Record only: vignette/CA/kaleido/twirl are raw-UV radial ⇒ ellipses at 11.69:1, all default 0.

**AA25 — 🎞️ HIS ARCHITECTURE RULING: THREE 10-MINUTE PARTS, and 🚨 THE PINNED PATH NEVER FORMS THE HOLE AT LAUNCH.** `[RESTATED 2026-09-04 15:19:48 → BOARD_BLACKHOLE.md §AG.1: "never ON THIS BINARY" — clean PINNED 0/17 vs UNPINNED 16/16, INVERTED by one extra small buffer; the pin is a layout, not the cause]` `[HIS WORDS 2026-09-04 ~00:1x]` *"three parts each 10 mins. waaaaaaaaaaay more digestible. drift wont get toooooo crazy. more easy to change stuff on the go. edit it if its shit. re render if somethings not adding up then waiting for a full 30 min file every time."* ⇒ live-vs-offline drift is a 10-minute question; 540 GB arrives as three ~180 GB parts; nothing in S6/S8 redesigned unasked. 🚨 `[MEASURED n=9/8]` **S8 renders pinned, and pinned launches formed a seed 0 of 9 times tonight while unpinned formed 8 of 8** — the show's own render path cannot form the hole at launch on this code. Mechanism unfound by reading; **`docs/BOARD_BLACKHOLE.md` §AF** carries the runs, the six killed hypotheses and the three roads. His call.

📖 **AA26 — THE SHOW IS RENDERABLE, THE RIDE IS BUILT, AND DELIVERY IS SOLVED (BRAIN, 2026-09-04 15:21:37).** ⭐ **COLD START FOR ALL RENDER WORK IS NOW `docs/SPACE_SYNTH_LIVE.md`** — his order, *"this must be the bible of the day"*: event, scale, pipeline, economics, and every learning with what it cost. Read it before this board for anything render-shaped.
- 🎬 **FRAME TIME HOLDS OVER A 10-MINUTE CAPTURE.** `[MEASURED n=600 blocks, 18,000 frames]` last ~30 min of wall clock drift **−0.39%**; all instability is the first ~20% (formation). 3 slices ffprobe'd exact, 0 writer failures, RSS flat 2337→2098 MB. 🚩 **A 60-SECOND TEST REPORTS THE OPPOSITE** (+121%, "degrades severely") because it measures only the transient — his order for ten minutes is what made the answer correct.
- 🎚️ **S6 SHIPPED — `14757d5` (camera) + `ff1fdc0` (midi).** Absolute setters `setZoomAbs`/`setTiltAbs`/`setPhiAbs` (targets only, spring untouched), CC map for zoom/tilt/phi/exposure/fluid/glitch/chromatic/isco/phase, **14-bit MSB+LSB pairs**, **receiver-side LSB latch** with a first-MSB fallback so a 7-bit Ableton source still moves, and **CC33 = θ range selector** (raw 0 ⇒ 2π orbit, nonzero ⇒ π/2 tilt). `[MEASURED]` his acceptance test passed **6/6** — every parameter 0→127→0 with the mapped value returning exactly, 2,772 `[MIDI-MAP]` lines.
- 🚩 **THE RIDE LAW, and it cost two takes.** `[HIS WORDS 2026-09-04 ~12:28]` *"its liek back and forth zoomies not one consistent ride"*. `[MEASURED n=133]` 7-bit gives one camera update per **30 output frames** against a **15-frame** settle — 134 lunges with dead stops. 14-bit gives median gap **0.00 frames**, 1.08 updates/frame, **43×**. ⚠️ **AND THE RANGE MUST MATCH THE TRAVEL:** a 90° sweep mapped over `[0,2π)` uses a QUARTER of the counts ⇒ one step per **150 frames**; mapped over `[0,π/2]` it is **38**. ⛔ Not fixes: resending a 7-bit value; going linear (spreads steps to 42 frames, *worse*).
- 🌀 **THE ORBIT ROLLED THE PICTURE — HIS CATCH.** `[HIS WORDS ~14:57]` *"the rotation seems wrong like the axis its not what i wanted"*. `[READ camera.h:263]` `refUp` is **θ-derived**; we made θ the orbit angle at φ=π/2, so `screenUp` swept a full 360° and the disk sat **vertical**. `orbitUpFix` pins `refUp` to the disk normal `(0,0,−1)` for orbit only — verified numerically at θ=0/90/180/270. ⚠️ **A θ sweep at φ=0 (edge-on→face-on) needs NO fix** — that is the elevation pivot the original derivation is for.
- 📦 **DXV3 DELIVERY IS SOLVED AND PROVEN.** `[HIS WORDS ~14:5x]` *"yoo thevideo runs in resoulume smootoooth"*. ffmpeg's `dxv` encoder emits **DXT1 "Normal Quality, No Alpha"**, MOV-tagged `DXD3`; `ffmpeg -i X.mov -c:v dxv -pix_fmt rgba`. `[MEASURED]` **L 1.35 / C 1.44 MB/frame ⇒ ~3× SMALLER than ProRes** — three 10-min parts ≈ **223 GB**, not ~690. Encodes at ~55 fps ⇒ ~16 min per part. ⛔ **My "~89 GB, bigger than ProRes" was WRONG** — it assumed fixed-rate DXT1; the encoder uses LZ-style back-references and crushes this footage's black. ⭐ **The 16-px padding worry did NOT materialise**: the 5340 centre slice encoded at 5340 and played — no crop, no re-slice, no seam.
- ⏱️ **RENDER ECONOMICS.** `[MEASURED]` 3-min zoom-only **27 min = 9.0× realtime**; with four post-FX faders sweeping **36 min = 12.0×**; 10-min static-camera **63 min = 6.3×**. ⇒ a 10-min part at the zoomed-in opening ≈ **90 min**, three parts ≈ **4.5 h**. The opening close-up is the expensive part.
- 🚨 **TAKE 4 IS BUILT AND HAS NEVER RUN.** Bundle 2026-09-04 15:13:59 — chord (C4/E4/G4, 600 frames) then tilt 0→90° and zoom, landing together at frame 5399, phase held 0. **Everything in it is verified against SYNTHETIC LOGS ONLY.** Smoke **900** frames before spending 27 min — the chord holds the camera static through 600, so a 300-frame smoke starts no tilt and would pass while testing nothing.
- 🎞️ On disk: `~/Desktop/sweep/take2_3min_{L,C,R}.mov` (**KEEP**) + `dxv3_test_C_10sec.mov`. Takes 1 and 3 deleted on his order. ⚠️ **A smoke artefact left on his Desktop reads as a failed deliverable** — he opened `take3_smoke_*.mov`, saw 10 s and reported the take had ended.

🎬 **AA27 — SEVEN TAKES RENDERED, AND THE COST DRIVER IS THE NOTE, NOT THE SPIN (OPUS, 2026-09-04 19:01:04).**
⭐ Detail lives in `docs/SPACE_SYNTH_LIVE.md` §4/§5/§6.13-6.16 — read it, not this row.
- 🚨 **A HELD NOTE RENDERS 4–6× FASTER THAN A RELEASED ONE.** `[MEASURED n=2 pairs, one variable]` take6 **48.6 → 8.52 fps** and take7 **49.3 → 13.24 fps** when the only change is the note-off frame; output grew 74 → 90 GB on the same shot. A sustained voice parks the field; letting go lets it evolve. **Note behaviour is a render-budget term.**
- ⛔ **"THE SPIN COSTS 3.3×" IS RETRACTED.** The controlled pair (same 3 s notes, same width, 5400 frames) is `take4rev_full` **15.17** vs `take5_spin` **13.64** fps ⇒ **11%**. The 3.3× compared a spin take against a SUSTAINED-note take.
- 📐 **CENTER-ONLY TEST RENDERS RULED IN — 1.34×** `[MEASURED n=1 pair, matched framing]` and it is a **TRUE CENTER CROP**: `perspectiveMatrix` (`renderer.mm:6186`) fixes vertical FOV, so tan(horizontal half-angle) scales exactly with width. ⛔ My "1.37×" from two full takes is RETRACTED — those runs had different note patterns. ⚠️ Dropping SLICES saves almost nothing; the three are crops of ONE render (`show_capture.mm:159`).
- 🎚️ **CC-DRIVEN SPIN SHIPPED `f0cac86`** — CC34/66 14-bit, 0 = stopped (the app default, so a dropped CC fails safe), 16383 = `kSpinMax`. Override sits AFTER the drag/clamp because no arrow is held in an offline render. `[MEASURED]` 87 `[MIDI-MAP] spinRate` lines and `[CLUSTER] spin` 0% → 1% at the predicted value.
- 🚩 **TWO LAUNCH FAULTS THAT COST TAKES:** gate the driver on `[MIDI] Listening` (line 46), NOT `[CAPTURE] ARMED` (line 21) — take 4's smoke ran `noteOn 0 / noteOff 3`; and clamp every note-off to `maxFrames - 1` — takes 6 and 7 v1 ended with their last note never released. **Check noteOn/noteOff COUNTS on every take.**
- 💾 **THE SSD IS exFAT** — `cp -p` fails at `chflags` AFTER writing the bytes and returns non-zero. A 17-file move reported 17 failures while the drive filled 69 → 230 GB. Plain `cp`, verify by size.


↩️ **AA22 — A COLLISION AND A RETRACTION, both recorded because the diff cannot show them.**
- **BRAIN RETRACTS "IT IS NOT THE PIN".** At 2026-09-04 ~00:2x I told him the resolution pin was not implicated in the missing black hole, reasoning that FABLE's 23:59 pinned run grew a 5,374 M☉ body. **That body grew AFTER a note; §AF.1 measures formation AT LAUNCH and finds PINNED 0/9 vs UNPINNED 8/8 on the same binary and spawn.** His *"i fele liek the bh didnt survie the scaling eitherl"* was correct and my ruling was wrong. ⇒ **never answer a formation question with a post-play sample.**
- **BRAIN RETRACTS THE AUGUST COMPARISON.** I compared tonight's silent run against `SPACE-SYNTH-TUBE/logs/A1_soak_1x_silent_20260807_125201.log` — **a different build**. `[HIS WORDS 2026-09-04 ~00:3x]` *"why dafuuuq would I use a log from a different build lol"*. **A cross-build log is not evidence of a regression; the 41.7% / 1.7% / 331,426 M☉ column must never be quoted.** Baselines come from THIS tree's own history.
- 🧮 **AND THE COUNTING TRAP THAT INFLATED MY NUMBERS — §AF.6:** every status line is written **twice** (`Logger::log` as `[DEBUG] FPS…` and `printf` as `FPS…`), so counting lines containing `% in` **doubles the sample count**. My "96.4% in by sample 147" is ~2× the real sample index. Means and maxima are unaffected; **counts and elapsed-time inferences are not.**
- 🚩 **PROCESS — TWO WINDOWS WROTE `docs/BOARD.md` INSIDE THE SAME MINUTE (02:29–02:31) AND EACH CLOBBERED THE OTHER.** FABLE's §AA16–AA18 rows were overwritten by BRAIN's read-modify-write and are **LOST from the working tree** (never committed); FABLE's stamp survived and described rows that were no longer there. Their substance is preserved in §AA16/AA17/AA20 and in `BOARD_BLACKHOLE.md` §AF, which was untouched. ⇒ **ONE WINDOW OWNS A BOARD FILE AT A TIME. Announce the fold, hold it, release it** — the same discipline as the app token.


🎥 **AA28 — THE POV SHAKE WAS THE PERSPECTIVE NEAR PLANE, NOT THE CAMERA. AND POV ZOOM READS INVERTED TO HIM. (FABLE, 2026-09-05 11:58:00; commits `0cac109`…`38170d5`, verified at `41609e1`.)**

| # | Finding | Proof | Where |
|---|---|---|---|
| AA28.1 | **Root cause of takes 6/7 POV "mad shaking" and the live POV "insane flicker": `perspectiveMatrix(…, 0.001f, 5000.0f)`.** NDC depth ≈ 1 − n/z; with n = 0.001 every particle beyond a few units lands within float32 spacing of 1.0 (`Depth32Float`, `renderer.mm:851`), so at z = 1000 two stars < ~60 units apart share one depth value and the star pass `Less` test against the depth prepass (`renderer.mm:1326`, `:4699`) flips per frame by thread order. Ortho maps depth linearly over ±5000 — never had it. **Fix: near 1.0, far 20000** (`38170d5`). | `[READ main.cpp perspective branch]` + `[HIS WORDS 2026-09-05 01:02:12]` *"amazing i think its gone"* on the live app in the identical config, one variable changed | `src/main.cpp` `perspectiveMatrix(` call, `src/render/renderer.mm:851/1326/4699` |
| AA28.2 | **The camera is exonerated.** New `[CAMF]` per-frame instrument (`f09609f`): in a 900-frame POV smoke the zoom spring output `drho` held 0.069–0.077 (±5%) while the picture stuttered ±30%; `kZetaZoom = 1.00f` (critically damped — BRAIN's "underdamped 0.70" was the ORBIT constant). Sim clock exact (`[OFFLINE] diff −2e-6 s`). His static-hold test (driver mode `c`): camera frozen at rho 927.45825 to 1e-5 from f450, field still stepped. | `[MEASURED n=3 runs: camf_b_smoke, camf_c_hold, camf_b_4x]` + `[READ camera.h kZetaZoom]` | `src/core/camera.h` `kZetaZoom`, `src/main.cpp` `[CAMF]` line |
| AA28.3 | ⛔ **Sprite size was a red herring.** `SS_REF_HEIGHT=420` (4x sprites) at 5340 centre read *"that was looking good"* (21:18), but the same at full width was *"crazy shake is back"* (2026-09-05 00:54). My per-star "whole-pixel step" measurement was a tracker artefact (dense-background centroid bias). The 1-px floor (`render.metal:1388`, `:2539`) is REAL but is the Chladni dashed-lines item (§AA row for 2026-09-03), not the shake. **His ruling 11:0x via BRAIN: no point-spread floor before Cologne.** 4x stays an env for the look. | `[HIS WORDS 2026-09-05 00:54]` + `[MEASURED]` retracted, see handoff §5 | `render.metal:1388`, `:2539` |
| AA28.4 | **POV zoom reads INVERTED to him.** *"cant believe it was that easy.. okay now we know pov is inverted"* after a rho-50 launch: his "all the way out" = **rho 50** (camera at the core, cloud all around), his "in" = **rho 5800**. Geometry: the rest cloud reaches maxR 61–76 sim × plate 100 ≈ 7600 world, so no POV distance is outside it. **Never argue geometry against this; set rides by it.** The trackpad pinch is wired to NOTHING (no magnify handler in `window.mm`); only scroll zooms. | `[HIS WORDS 2026-09-05 01:37]` + `[READ window.mm scrollWheel only]` | `src/ui/window.mm:257` |
| AA28.5 | **kMaxRho 2000→5800** (`0cac109`): ortho far end shows half-height 2400; perspective at 45° shows 0.414·d ⇒ d = 5794, derived. The 14-bit zoom CC now spans 50..5800; every older ride's rho print is stale (cosmetic). | `[READ camera.h kMaxRho, main.cpp CC20/52 map]` | `src/core/camera.h`, `src/main.cpp` `case 52` |
| AA28.6 | **`SS_LUM_CEIL=<v>`** sets the Lum Ceiling fader at launch, `[LUM]` line as proof (`a94e76f`). His order 01:39 *"for all runs lumen ceiling 520"*. | `[READ main.cpp env block]` + log lines in takes 8/9 | `src/main.cpp` after `SS_ORTHO` |
| AA28.7 | **Takes 8 and 9 rendered and delivered.** `take8_pov_octaves_50to2925` (rho 50→2925, C every minute rising octaves 60/72/84/96/108, held 3 s) and `take9_pov_chords_2925to50` (rho 2925→50, Cmaj/Dmin/Emin/Fmaj at minutes 0–3 held **6 s** — *"chords held twice as long as the notes"* 01:51 — minute 4–5 silent). Both 9000 f, 19644×1680 ×3 slices, POV 45°, default pose (NO theta/phi CC sent), `SS_REF_HEIGHT=420 SS_LUM_CEIL=520`. Every note/chord on+off confirmed in the app logs. DXV3 on `/Volumes/LOSTINSPACE/JAMAL/` (all six DXD3, 9000 f), ProRes originals in `~/Desktop/sweep/`. Driver modes `d`/`e` in `logs/midi_ride_shot.mm` — ⚠️ `logs/` is gitignored, the recipes exist only on this Mac. | `[MEASURED ffprobe ×6]` + app logs | `logs/run_shot.sh`, `logs/midi_ride_shot.mm` |
| AA28.8 | ⚠️ **Two-window commit collision 11:53–11:54.** BRAIN committed the tree by hunk while FABLE's by-content staging ran under `/handoff` ⇒ `25bbfc7` was a regression, reverted by `41609e1`. Rule already exists (🔑 token handover); the gap is that `/handoff` in one window is not visible to the other. | `[READ git log]` | — |
| AA28.9 | **Both show takes carry the fix — by ordering, not by report.** Bundle built 2026-09-05 01:40:07 (near 1.0 + far 20000 + SS_LUM_CEIL); take 8 log ends 01:48:35, take 9 01:58:37. 75 `[PROFILE/120f]` windows per take, ZERO with `Compute avg 0.00` ⇒ the sim never paused. All nine DXV3 files on LOSTINSPACE carry `DXD3` at 7152/5340/7152 with exact counts (take4 5400, takes 8/9 9000). Depth derivation reproduced independently: dz_ndc/dz = 1e-9 at z = 1000 vs a ulp of ~6e-8 ⇒ ~60 world units per depth step. (BRAIN, re-measured 11:5x.) | `[MEASURED n=6 slices + 150 profile windows]` | logs in `~/Desktop/sweep/`, bundle mtime |
| AA28.10 | **take4_chord_full (2026-09-04 19:55–19:59) VERIFIED — the ortho control the shake was measured against.** 5400 f in 264 s = 20.45 fps, 63.4 GB ProRes; chord on f0, off f600.6 vs target 600; landed zoom14 = 0 (rho 50) and theta14 = 16383 (90.00°) together; theta gate p50 38.0 frames ⇒ the [0, π/2] mapping held; 45 profile windows, none paused; three slices ffprobe exact. DXV3 on LOSTINSPACE (00:45–00:48). (BRAIN.) | `[MEASURED]` | `~/Desktop/sweep/take4_chord_full.*`, `logs/run_take4_chord.sh` |
| AA28.11 | **Hunk staging without `git add -p`:** `/Users/airy/.claude/jobs/dc2fe2fd/tmp/hunk.sh` (BRAIN) splits `git diff -U3` by `@@` and `git apply --cached`s one hunk — always stage hunk 1 and re-run, indices renumber after each commit. FABLE's alternative: write intermediate file states by content and `git add` each. Either beats what happened at 11:54. | `[READ]` | — |

🎥 **AA29 — THE ORBIT TAKE, RENDERED THROUGH THE APP'S OWN REPLAY; A THETA SPIN BUG FOUND BY CHECKING THE STREAM; THE SONG SHOT PLANNED AND CANCELLED. (FABLE, 2026-09-05 15:25:00; source commits `8182846` `7ea4fe8` `acda80f` `23222fc`.)**

| # | Finding | Proof | Where |
|---|---|---|---|
| AA29.1 | **`SS_REPLAY` is a delivery transport.** A generated "SPACE SYNTH take v1" file (`scratchpad/gen_take.py`) is applied by S4 replay at frame = ceil(t·30) through the same `onMidi` as live MIDI, 14-bit latch included. No driver, no marker extrapolation: beat N is frame 15N by construction. `scratchpad/check_replay.py` diffs every applied `[REPLAY]` line against the file. | `[MEASURED n=6 runs: 2146/2146, 3191/3191, 2424/2424, 3191/3191, 20866/20866 events, 0 mismatches, 0 frame-late]` | `src/core/take_replay.cpp`, `scratchpad/gen_take.py`, `scratchpad/run_replay.sh` |
| AA29.2 | 🚨 **An absolute theta target above π made the camera spin a full turn every 7 frames.** `update()` co-wraps the ACTUAL into (−π, π] and shifts the target by 2π; `setTiltAbs` re-asserted the raw value every frame ⇒ error ≈ 2π after each wrap. Take 3's orbit (0→2π) had it in its second half — a second cause behind his *"the rotation seems wrong"*, next to the roll (§AA26). Modes a/b (π/2 range) and takes 8/9 (no theta) could never hit it. **Fix `8182846`:** `tgtTheta = theta + wrapPi(thetaAbs − theta)`. | `[MEASURED]` theta cycled −π..π at f=4, 11, 18, 25… (smoke_song_drop); after the fix the 200° excerpt tracks within 0.003 rad from f=30 | `src/core/camera.h` `setTiltAbs`, `coWrap` |
| AA29.3 | **`SS_CAM_THETA` / `SS_CAM_PHI`** (radians) launch the camera IN a pose: the orbit needs φ = π/2, the default is 0, and there was no launch env for the angle (bible §5) ⇒ frame 0 sprang 90° over 15 frames. `7ea4fe8`. | `[MEASURED]` f=0..15 hold θ, φ to 1e-4 with both at π/2 | `src/core/camera.h` `reset()` |
| AA29.4 | **14-bit exposure pair CC22 MSB + CC54 LSB** (`acda80f`), same latch as zoom; until a CC54 is ever seen every MSB still applies coarse ⇒ 7-bit senders unchanged (verified: 7-bit pump 0.72/0.78/0.96 on the rebuilt bundle). CC54 was unclaimed. Built but no delivered take uses it. | `[READ main.cpp case 22/54]` + `[MEASURED]` | `src/main.cpp` `case 22`, `case 54` |
| AA29.5 | **take10_orbit_wide DELIVERED** — ORTHO, `SS_CAM_RHO=50 SS_CAM_THETA=π/2 SS_CAM_PHI=π/2 SS_LUM_CEIL=520`, camPhi 32 (orbit + `orbitUpFix`), thetaRange 0, zoom p² rho 50→**5800** (his ruling *"Wider one"* 2026-09-05 ~13:5x), θ 360° from π/2, no notes, 5400 f, 19,644×1680 ×3. 20866/20866 events, 0 paused windows, end θ = π/2 (full turn), rho 5790. **65 GB ProRes (19/23/23), DXV3 on `/Volumes/LOSTINSPACE/JAMAL/` ×3 DXD3 5400 f.** Bundle **14:08:19** — a DIFFERENT binary from takes 4/6/7/8/9 (11:57:55). | `[MEASURED ffprobe ×3, check_replay]` | `scratchpad/takes/orbit_wide_5400.take` |
| AA29.6 | ⏱️ **The orbit is render-bound and 2.2× take 3:** Render+PostFX 253–349 ms/frame vs take 3's 152–158 (compute 46–48 vs 33–37); ~24 min wall for 5400 f. BRAIN's suspect: the per-device-pixel smear taps (`e750a73`, §AA24 "cost at 256 taps UNMEASURED") on a continuously moving camera at 19,644 wide; `pixelStretch = max(fader, bhStrength)` keeps the smear on while the hole is up (`renderer.mm:5410`). ⚠️ This run's fps is contaminated: the 271 GB masters copy read the same volume for its first 7 min. **Controlled pair (smearShutter 0 vs default, 900 f centre) NOT run — his call, smear is a look.** | `[MEASURED PROFILE windows]`, cause `[HYPOTHESIS]` | `src/render/renderer.mm:5410`, `postfx.metal` smear |
| AA29.7 | 🎵 **THE SONG SHOT — planned, generated, smoked, then CANCELLED** `[HIS WORDS 2026-09-05 ~14:30 "Actually don't do the music run at all skip it"]`. Song "JAMAL ONE WITH THe NIGHT V2.mp3": 227.997 s, **120.0 BPM measured** (kick-band onset autocorrelation; comb refine 119.99) ⇒ 15 f/beat exactly; 613 low-band onsets, 355 on the eighth grid (phase 0.0 f); sections from per-bar energy (intro 0–30 s, breakdown 128–144 s, drop 144 s, plateau 162–192 s, outro 216 s). Plan v1 in `gen_take.py SECTIONS`: ortho 360° orbit, rho per section eased over 8 bars, exposure pump on the 355 grid kicks (attack 1 f, release τ 6 f, depth 10–30 %, floor 0.70), **one held chord per section = 15 note-ons against 613 onsets — the cost law (§AA27) decision, not an aesthetic one**, no glitch. Key measured (tuning −33 cents, Eb/Bb centre, no third) and **moot: the synth is not started offline (`[OFFLINE] audio engine NOT started`), notes only shape the field**. Audio would have been muxed into the CENTRE slice (`ffmpeg -map 0:v -map 1:a -c:v copy -c:a aac`). Drop smoke with audio at `~/Desktop/sweep/smoke_song_drop2_WITH_AUDIO.mov`. | `[MEASURED]` for the numbers; plan `[HIS WORDS "song plan good"]` then cancelled | `scratchpad/gen_take.py`, `logs/takes/song_*.take` (gitignored, regenerable) |
| AA29.8 | 💾 **Masters of takes 4/8/9 (271 GB ProRes + logs) COPIED to `/Volumes/LOSTINSPACE/JAMAL/MASTERS_PRORES/`**, byte-size verified per file, ~660 MB/s; **NOT deleted locally — his call.** Takes 6/7 (5-min POV, BRAIN) converted to DXV3 on the drive 14:47–15:08, all six probed DXD3 / 9000 f. **Delivered on LOSTINSPACE: 18 files, 33 minutes** (takes 4, 6, 7, 8, 9, 10); +7 min on T9 (take4rev, take6 ortho) ⇒ 40 min. T9 returns later with take2/take5/take7-ortho ProRes still to convert (+10 min). | `[MEASURED ffprobe ×18, stat]` | drives |
| AA29.9 | ⚠️ **Two-window job collisions, twice more today:** the takes 6/7 DXV3 conversion was started by BOTH windows (14:39 BRAIN, 14:47 FABLE; a 6.4 GB partial R was overwritten by the owning run), and `/handoff` was run in both windows at once (15:1x). Rule adopted by both windows: **whoever takes a job says "taking it" and the other confirms before touching anything** — app token, tree, conversions, docs. | `[READ logs]` | — |

🎬 **AA30 — THE NEAR-PLANE FIX IS A FACTOR, NOT IMMUNITY: POV HAS A ZOOM BOUND. TAKES 6 AND 7 REDONE AT FIVE MINUTES. (BRAIN, 2026-09-05 15:2x; source commits `0cac109` `1460d40` `a94e76f` `f09609f` `38170d5`.)**

| # | Finding | Proof | Where |
|---|---|---|---|
| AA30.1 | 🚨 **A POV SHOT PAST rho ~2925 IS EXPECTED TO SHAKE — the bound is derivable, not a mystery.** §AA28's near 0.001→1.0 removed the shake at the framings takes 8/9 use and NOT at the framing modes a/b use. Depth step = `ulp·z²/near`, float32 ulp near 1.0 = 5.96e-8: **near 0.001 @ z=1000 → 59.6 units** (the original defect) · **near 1.0 @ z=2925 → 0.51** · **near 1.0 @ z=5800 → 2.00**. Neighbouring stars inside one step share a depth value and the star pass `Less` test against the prepass flips by thread order — the SAME mechanism, 4× further out. ⚠️ I checked STALE-ARTIFACT FIRST: bundle 11:57:55 built from source carrying `1.0f, 20000.0f`, process started 13:02:06 from it. Not stale; the fix was live and it still shook. ⛔ **ORTHO IS EXEMPT** — `orthoMatrix` maps depth LINEARLY over ±5000, which is why §AA29's orbit is safe at 5800. | `[HIS WORDS 2026-09-05 ~13:0x]` *"i see that the take i shsaking right now"*, then *"8 and 9 look amazing no shakies all good"* + `[READ main.cpp perspective branch, renderer.mm:851 Depth32Float]` | `src/main.cpp` perspective branch |
| AA30.2 | **HIS FIX IS A CAP, AND IT IS NOT A FIX.** `const double kOutCap = 0.5` in `midi_ride_shot.mm` applied to modes a/b ⇒ they land at zUnit 0.5 = **rho 2925**, exactly where takes 8/9 live. ⭐ **The real fix is reversed-Z depth** (far→0, near→1, compare Greater) — NOT a today change, NOT started. | `[HIS WORDS ~13:1x]` *"2 yeah for that"* | `logs/midi_ride_shot.mm` |
| AA30.3 | **TAKES 6 AND 7 REDONE AT 5 MINUTES, DELIVERED.** `take6_pov5m_mid_in_out_still` 9000 f / 545.5 s, notes 0-90 + 3600-3690; `take7_pov5m_in_out_4notes` 9000 f / 505.3 s, notes on the quarters 0/2250/4500/6750. Both land zoom14 ~8192 (rho 2925) + theta 90°. Env identical to takes 8/9: `SS_ORTHO=0 SS_FOV=45 SS_REF_HEIGHT=420 SS_LUM_CEIL=520`. | `[MEASURED]` all six slices ffprobe 9000 f at 7152/5340/7152×1680; 75 `[PROFILE/120f]` windows each, **ZERO** `Compute avg 0.00` | `~/Desktop/sweep/` |
| AA30.4 | 🚩 **TWO LATENT DRIVER FAULTS, both fixed.** (a) modes a/b hardcoded `maxFrames = 7200` **and** their leg boundaries as literal frames (2700/5400) — changing take length silently desynced the legs from the take; legs now derive from `rideFrames`. (b) the `take complete` line printed **`rho=2000`** from a stale kMaxRho span (1950) while the camera actually landed at **5800**; the per-tick line was already correct at 5750. ⭐ **A log line that lies about where the camera went is worse than no line.** Verified against a synthetic 9000-frame log BEFORE spending a render. | `[READ logs/midi_ride_shot.mm]` + synthetic dry run: mode a 2 notes, mode b 4, both complete at zoom14 8192 | `logs/midi_ride_shot.mm` |
| AA30.5 | 📦 **DELIVERED: 18 DXV3 files, SIX takes, 33 minutes** — 4 (5400 f), 6 (9000), 7 (9000), 8 (9000), 9 (9000), 10 (5400). Independently ffprobed by me at 15:0x: every file `DXD3` at 7152/5340/7152. ⚠️ **THE SET SPANS TWO BINARIES** — takes 4/6/7/8/9 from the **11:57:55** bundle, take 10 from **14:08:19** (which carries `8182846` `7ea4fe8` `acda80f`). Re-rendering 6/7 byte-for-byte needs the tree WITHOUT those three. "Same tree" is not true of this set. | `[MEASURED n=18 ffprobe]` | `/Volumes/LOSTINSPACE/JAMAL/` |
| AA30.6 | 🚩 **A BY-HUNK COMMIT REWRITES SOURCE mtimes AND MAKES PREFLIGHT CRY STALE ON A CURRENT BUNDLE.** Twice today, to both windows: `src/core/camera.h` mtime 15:21:23 is the STAGING, not an edit — both camera commits carry that same second, and the 14:08:19 bundle's CONTENT is unchanged. The skill's red flag is *"the change did nothing → stale artifact first"*; **this is its inverse and costs a re-read every time.** | `[MEASURED]` mtime vs `git log --date` on the same file | preflight §3 |

⛔ **DEAD ROAD — DO NOT RE-DERIVE:** the POV shake is **not** the camera spring and **not** perspective parallax and **not** the 1-px sprite floor. All three were mine, all three are retracted at §AA30.7, and §AA29.2's theta spin is a *different* bug in a *different* projection. The only live mechanism is depth precision.

| AA30.7 | ↩️ **RETRACTED, MINE** | |
|---|---|---|
| "the shake is the camera spring / perspective parallax" | I quoted `kZetaOrbit` **0.70** at a zoom spring that is `kZetaZoom` **1.00** — critically damped (`camera.h:110-111`) — and measured with phase correlation, which locks onto the static core of a star field and under-reports. FABLE's per-star tracking was the right instrument. | |
| "the 1-px sprite floor is the root" | Killed by his own eyes: the 4×-sprite run at full width still shook, *"crazy shake is back"*. The real point-spread floor stays a Chladni dashed-lines item, unrelated. | |
| "POV renders black" | A 150-frame smoke on a field that had barely formed. The finished POV takes read **YAVG 169** at f900 against video black 64. | |
