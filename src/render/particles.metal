    ...
    uint id [[thread_position_in_grid]])
{
    ...
    if (numVoices > 0 && baseMass < 1000.0f) {
        ...
        // Only active during play (not silence). During silence the disk
        // confinement forces handle structure. The spherical shell target
        // fights the flat accretion disk and creates orbital yoyo rings.
        if (!isSilence) {
