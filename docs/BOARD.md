...
🚨 **AA7 — THE FULL-RES BLOCKER, and it is a LAYER limit, not a memory limit.** `[MEASURED 18:22–18:25, FABLE]` Every offscreen texture type allocates **and rasterizes** at 19,644×1680 (RGBA16F mipmapped, RGBA16F, Depth32F, RG16F, RGBA8; clear+readback exact) and ProRes 422HQ/4444/HEVC sessions open at that size — but **the presenting `CAMetalLayer` refuses any drawable above 16,384**: `nextDrawable` nil, `drawableSize` reads 0×0, and the frame's final blit `copyFromTexture:drawable.texture` dereferenced nil ⇒ **SIGSEGV** (crash report 18:22:51; his words *"it crashed lol"*). Metal's own descriptor validation **aborts** above 32,768 (report 18:24:10). ⇒ **S8 must render the final post pass into an OFFSCREEN target of the pinned size and present a ≤16,384 preview; the capture reads the offscreen target.** Guard built 18:30:36, **NOT LAUNCHED** — his interrupt on the launch stands. **Half res works end to end today.**
...
**Still unbuilt, in FABLE's stated order:** **S8** capture (offscreen pinned target → `CVPixelBuffer` → three `AVAssetWriter`s cropping L 7152 / C 5340 / R 7152, ProRes 422 HQ) — **this is the step that produces the first frame; 0 frames exist today** → **S6** the CC→parameter registry from `docs/DESIGN_2026-09-03_MIDI_MAP_AND_LINK.md` (apply **outside** `if (showHUD)`; refuse mapping the marker CC) → **S7** slew on the frame clock. **On his go.**
