...
struct AppState {
  ...
  bool  uiTogRayMarch = true; // bit19 — DEFAULT **ON** as of 2026-08-17 15:58:12, his order ("open it with the switch on"), after the reason it was switched off was REMOVED. ⭐ THE ORANGE IS DELETED (render.metal, the emission line of bhmarch_fragment). The hue is now blackbodyRGB(g·T(r)) with T(r) the Shakura–Sunyaev profile (r/r_in)^(−3/4)·[1−√(r_in/r)]^(1/4), r_in = ISCO = 3 r_s, anchored at 6500 K = DNGR's own white-balance point — his call "a mix between the nasa and interstellar vibe": NASA's radial structure at Interstellar's colour temperature. Runs 6500 K at the inner ring to 1322 K at 60 r_s. g (Doppler + gravitational, ONE factor) beams it as g³. ⚠️ Still a DECLARED TRANSFER, not a claim: our disc measures h/r = 0.746, a thick RIAF at ~4e10 K that emits in X-ray and has no true RGB. The 128³-box-average resolution objection below is UNADDRESSED. ── PRIOR REASON FOR OFF, kept for the record (2026-07-28 09:32:18). Jamal: "this orange shadow of the blackhole is super old and must leave asap." Its only colour WAS a hardcoded float3(1.0, 0.55, 0.25) (the comment said render.metal:2474; that line number had decayed) — no temperature input, and no temperature grid exists anywhere in the renderer, so it could not be anything but orange at some brightness. It also cannot ever produce Chladni structure: its output is a box-average of a 128³ density grid, while the sprites draw 2M+ bodies at sub-pixel precision. Proven 2026-07-26: trilinear sampling of that grid changed nothing he could see, and the two renderers were being ADDITIVELY OVERLAID the whole time (two pictures of one disk ~100x apart in resolution = "it doesn't connect to the rings"). The 2026-07-26 21:20 A/B settled it — lens bit8 ON, march off: "ITS FINALLY THE CORRECT FEEL". The SHADOW is unaffected: that is bit15 (metric shadow) plus the capture cull, both still on. Turn this back on only for a deliberate A/B, and never as the hole's renderer. Was DEFAULT ON (2026-07-25, "calculate the hole, don't put a lens there"). The metric-native ray-march: integrate null geodesics of the honest metric (r_s = emergent r_h) BACKWARD and ADD the emission of the REAL particle field (CIC hash grid) gathered along each ray. Rays winding over the hole pick up the disk's far side (the arch = inner/outer ring); captured rays stop gathering (shadow = absence). Additive — no black paint, so the withdrawn overlay disc cannot recur. This is what a flat lensed sprite could never do (one sprite ≠ 3 images). OFF = no metric emission (the sprite-lens/absence path only). emitScale hardcoded 1e-3 for now — the single brightness dial.
  ...
  float uiRayBcull = 7.0f;        // metric ray-march (bit19) EXTENT: impact-parameter cull in r_s. 2.6 (the shadow-only value) discarded every ray that would show the disk → the filled orange core. Widen to render the disk + its lensed rings. Higher = more disk but more pixels marched: marched screen AREA goes as bCull², at 256 steps per ray. Was 16.0 (and 40 = slider max was the live value at the 2026-07-26 handoff, measured 7-15 fps). 7.0 set 2026-07-26 20:41:03: MEASURED [GRAV] meanR=3.92 maxR=4.4 sim with camHorizonR (r_s) = 0.80 ⇒ ALL the matter sits inside 5.5 r_s, so 40 marched ~7x too wide in radius and 16 still ~3x — mostly vacuum. ⚠ THIS DEFAULT TRACKS THE COLLAPSED BALL. The same field 20 min earlier was a thick torus at meanR=33 sim = 41 r_s; if/when the §0.1 "keep the matter a DISK" work lands, 7 will CLIP the whole disk and this must be raised again (or, better, derived from the measured maxR instead of being a constant).
  ...
  float uiRayEmitLog = -6.0f; // -6.0 AGAIN 2026-08-17 17:35, but the dial CHANGED MEANING: it is now OPTICAL DEPTH, not an open gain. The 16:26 brown fill came from emit+= having no absorption, so a 60 r_s path summed without limit; the transfer now saturates at B(T) and the near surface dominates. At -7.5 the transfer is a numerical no-op (dTau~3e-8) so the pass stays invisible. Earlier note: — raising it was MY unrequested change and it produced the brown fill he rejected on sight ("the emission is this bs"). Brightness is HIS (2026-08-14 12:26). Prior note kept: -7.5 was tuned to make the HARDCODED ORANGE fade out; that orange is deleted, so the old value only hid the new temperature-graded emission. [MARCH] confirmed the pass DRAWING at -7.5 and still invisible. -6.0 is where the note below says the marched region actually reads. Expect the bCull box edge to show — that edge is the next problem, not a regression. ORIGINAL NOTE:     // log10 of the metric ray-march emission gain (bit19). Default −7.5 (2026-07-25 16:33: Jamal "always have to lower to −7.5ish for the orange cube to disappear"). −6 filled the bCull march box with a bright warm haze (the "orange cube/box" = the emission bounding region); −7.5 is where the fill fades and only the disk/lens structure remains. emitScale = 10^this. 1e-3 saturated the whole capture disc to a solid orange "yolk" 2026-07-25 14:20 (every ray hit the clamp) — sweep DOWN until the saturation breaks and the disk/lensing structure emerges. LOWER = dimmer.
