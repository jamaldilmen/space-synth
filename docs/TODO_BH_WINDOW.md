...
| **B-2** | **R2 fix — drop the in-loop clamp**, let the exact log-divergent `alpha(b)` LUT place the images so they pile toward `b_c` with a real density falloff. No new pass, no perf cost, sprite-native. The 2026-08-14 capture test (`along>0 && thEff < 2.598·rsW → cull`) already covers shadow-emptiness for the identical particle set, so the floor is redundant for that job. **Predicted:** the spike at 2.61 r_s spreads into a gradient and the empty 2.06–2.60 band stops being empty. Should also sharpen R5. ⚠️ **Risk:** the clamp may be doing numerical duty near the divergence; fallback is a clamp at 2.598 (the capture radius) so it stops being a pile-up 0.8% outside the cull. | 🔴 **TRIED 2026-08-26 AND REFUTED BY MEASUREMENT. Token spent, result is real evidence.** Guard moved to b_c+1e-4; three matched settled pairs, background identical to 4 d.p. so only lensed features moved. **The spike DID die (0.291→0.223, to background) but NO gradient replaced it** — the 1.82–2.67 r_s band went DOWN 5.1%, still flat; the 3.06 r_s feature lost 90%; **total flux fell 13–17% in every pair. Images are being CULLED, not redistributed.** ⭐ **MECHANISM: the clamp was doing real numerical duty.** α(2.62)=4.396 rad but α(2.5981)≈12 rad — 3× larger; an overshooting Newton iterate lands where α is enormous, the solve diverges, thEff collapses below the capture radius and `:1161` culls it. That is why light at 3.06 r_s died too. **The clamp was not only a floor. Removing it without replacing the solver destroys images.** ⟹ **The sprite-lens path CANNOT carry radial structure near b_c with this solver** — so (b) does not get the cheap annulus-only option on the strength of a salvageable sprite path. |
