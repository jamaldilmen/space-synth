...
**Commit at last verification:** `ae6ecdd` (2026-08-22 10:10:00). Rows were re-read in SOURCE this session, not carried. 🚨 **UNCOMMITTED WORK SITS ON TOP OF THIS SHA** — the 2026-08-22 session: BH6 (AMR to bit21), the metric-shadow default flipped OFF, the deflection LUT re-parameterised, and the playback axis unified. None of it is in any commit; no commit order was given. ⭐ Prior note kept: the 08-20 body plus the 08-21 S1/S2 work landed TOGETHER as `6b923fe` because `main.cpp`, `render.metal`, `renderer.h` and `renderer.mm` each carry both days — a KNOWN violation of ONE-CHANGE-AT-A-TIME, stated rather than hidden. If a regression appears in that range, attribution needs the diff, not the log. 🎪 **The show moved to COLOGNE 2026-09-05** (`docs/SHOW_2026-09-05_COLOGNE.md`); this board still says Berlin below. 📋 **`docs/TODO.md` is the cold start** — it carries this session's findings in full.
...
> |---|---|---|
> | ⭐ `SPACE-SYNTH-TUBE-killtube` (worktree, branch `kill-the-tube-2026-08-11`, HEAD `e853e18`) | **2026-08-17 17:45:51** | **THE LIVE ONE.** Everything through the 08-17/18 BH march session. Uncommitted: `CLAUDE.md`, `app_state.h`, `render.metal`, `renderer.mm`, the bundle, 2 boards, 2 docs |
...
>
> **Launch, do not rebuild.** Running now: **nothing** (`pgrep -x SpaceSynth` empty, checked 2026-08-18 21:07:00).
>
